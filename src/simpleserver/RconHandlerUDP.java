...
public class RconHandlerUDP implements Runnable {
	...
	public RconHandlerUDP(Socket s, Rcon r) throws IOException {
		    	...
		    	*/
		    	...
		    	if (!response.equals("")) {
			    	...
			    	for (i=0;i+4096<bbSend.capacity();i+=4096) {
			    		...
			    		send[4096]=0;
			    		send[4097]=0;
			    		parent.socket.setSendBufferSize(send.length);
			    		out.write(send);
			    	...
			    	bb.position(0);
		    		bb.putInt(packetSize);
		    		parent.socket.setSendBufferSize(bb.array().length+2);
		    		out.write(bb.array());
		    		out.write(0);
		    		out.write(0);
		    	}
